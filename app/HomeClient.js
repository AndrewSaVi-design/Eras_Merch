    ...
    <main className="min-h-screen bg-white flex flex-col items-center text-black relative font-sans overflow-x-hidden">
      ...
      {vista === 'colecciones' && (
        <div className="w-full flex flex-col items-center px-4">
          ...
          <div className="w-full max-w-2xl flex flex-col md:flex-row gap-4 mb-12">
            ...
            <button onClick={() => {
               artistasRef.current?.scrollIntoView({ behavior: 'smooth' });
            }} className="bg-black text-white px-8 py-4 rounded-2xl text-[10px] font-black tracking-widest">VER ARTISTAS</button>
          </div>
          ...
          <div ref={artistasRef} className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-5xl mb-20">
            {artistas.filter(a => a.nombre.toLowerCase().includes(busqueda.toLowerCase())).map(art => (
              <div key={art.id} onClick={() => { setArtistaId(art.id); setVista('productos'); window.scrollTo(0,0); }} className="cursor-pointer group flex flex-col items-center">
                <div className="aspect-square w-full rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-100">
      ...
      {vista === 'productos' && artistaActual && (
        <div className="w-full min-h-screen relative flex flex-col items-center">
          ...
          {urlFondo && (
             <div 
               className="fixed inset-0 z-[-1] bg-cover bg-center transition-opacity duration-700 opacity-40"
               style={{ backgroundImage: `url(${urlFondo})`, filter: 'blur(10px)' }}
             />
          )}
          
          <div className="w-full max-w-6xl p-6 mt-10">
            <button onClick={() => setVista('colecciones')} className="flex items-center gap-2 text-black/60 text-[10px] font-bold mb-10"><ArrowLeft size={16} /> VOLVER</button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {polosAMostrar.map(polo => (
                <div key={polo.id} className="bg-white/95 border border-gray-100 p-4 rounded-[2rem] flex flex-col items-center shadow-xl">
                  ...
                  {/* PRECIO EN NEGRO SÓLIDO */}
                  <p className="text-black text-[16px] font-black mb-4">S/ {polo.precio}</p>
                  ...
                  }} className={`w-full py-4 rounded-xl text-[10px] font-black tracking-widest ${notificacion === polo.id ? 'bg-green-500 text-white' : 'bg-black text-white'}`}>
