...
export default function HomeClient({ artistas = [], productos = [], banners = [], fondosMap = {} }) {
  ...
  const artistaActual = artistas.find(art => art.id.trim().toLowerCase() === artistaId?.trim().toLowerCase());
  const polosAMostrar = productos.filter(p => p.artista === artistaId?.trim().toLowerCase());
  const fondoUrl = artistaId ? fondosMap[artistaId.trim().toLowerCase()] : null;
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col items-center text-black relative font-sans overflow-x-hidden">
      ...
      <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" className="fixed bottom-6 right-6 z-[99] hover:scale-110 transition-transform">
        <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" className="w-14 h-14 shadow-2xl rounded-full" />
      </a>
      ...
      <div className="w-full max-w-4xl p-6 flex flex-col items-center relative">
        ...
        <div className="flex gap-6 text-gray-400">
          <a href="https://www.instagram.com/eras_merch" target="_blank" className="hover:text-black"><Instagram size={20} /></a>
          <a href="https://www.facebook.com/share/1AqEUYkZKL/" target="_blank" className="hover:text-black"><Facebook size={20} /></a>
          <a href="https://www.tiktok.com/@erasmerch" target="_blank" className="hover:text-black"><TikTokIcon size={20} /></a>
        </div>
      </div>
        ...
        <div className="w-full flex flex-col items-center px-4">
          ...
          {banners?.length > 0 && (
            <div className="w-full max-w-5xl mb-10 group relative">
              <div className="relative aspect-[16/11] md:aspect-[21/9] w-full overflow-hidden rounded-[2.5rem] shadow-xl bg-gray-50">
                {banners.map((url, i) => (
                ...
                <button onClick={() => setCurrentBanner(p => (p - 1 + banners.length) % banners.length)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/50 p-3 rounded-full opacity-0 group-hover:opacity-100 transition-all z-10"><ChevronLeft size={30}/></button>
                <button onClick={() => setCurrentBanner(p => (p + 1) % banners.length)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/50 p-3 rounded-full opacity-0 group-hover:opacity-100 transition-all z-10"><ChevronRight size={30}/></button>
              </div>
            </div>
          )}
          ...
          <div ref={artistasRef} className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-5xl mb-20">
            {artistas.filter(a => a.nombre.toLowerCase().includes(busqueda.toLowerCase())).map(art => (
              <div key={art.id} onClick={() => { setArtistaId(art.id.trim().toLowerCase()); setVista('productos'); window.scrollTo(0,0); }} className="cursor-pointer group flex flex-col items-center">
                <div className="aspect-square w-full rounded-[2.5rem] overflow-hidden shadow-sm hover:scale-105 transition-all">
      ...
      {vista === 'productos' && artistaActual && (
        <div className="w-full min-h-screen relative flex flex-col items-center">
          ...
          {fondoUrl && (
            <div 
              className="fixed inset-0 z-[-1] bg-cover bg-center transition-all duration-700 opacity-50"
              style={{ backgroundImage: `url(${fondoUrl})`, filter: 'blur(15px)' }}
            />
          )}
          
          <div className="w-full max-w-6xl p-6 mt-10">
            <button onClick={() => setVista('colecciones')} className="flex items-center gap-2 text-gray-600 text-[10px] font-bold mb-10 uppercase tracking-widest"><ArrowLeft size={16} /> VOLVER</button>
            <h2 className="text-4xl md:text-6xl font-black text-black text-center mb-16 uppercase italic tracking-tighter">{artistaActual.nombre}</h2>
            
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {polosAMostrar.map(polo => (
                <div key={polo.id} className="bg-white/95 border border-gray-100 p-4 rounded-[2rem] flex flex-col items-center shadow-2xl">
                  ...
                  <p className="text-black text-[18px] font-black mb-4">S/ {polo.precio}</p>
                  <div className="flex gap-2 mb-4">
                    {polo.tallas.map(t => (
                      <button key={t} onClick={() => setTallaSeleccionada({...tallaSeleccionada, [polo.id]: t})} className={`w-8 h-8 rounded-full text-[9px] font-black border ${tallaSeleccionada[polo.id] === t ? 'bg-black text-white' : 'text-gray-400 border-gray-200'}`}>{t}</button>
                    ))}
                  </div>
                  <button onClick={() => {
                  ...
                  }} className={`w-full py-4 rounded-xl text-[10px] font-black ${notificacion === polo.id ? 'bg-green-500 text-white' : 'bg-black text-white'}`}>
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
              {carrito.map((item, i) => (
                <div key={item.tempId} className="flex gap-4 items-center border-b pb-4">
                  ...
                  <div className="flex-1 text-[10px] font-black uppercase text-black">{item.nombre} ({item.tallaElegida})</div>
            ...
            {carrito.length > 0 && (
              <div className="pt-6 border-t mt-auto">
                <button onClick={() => {
                  ...
                  window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=Pedido:%0A${lista}`, '_blank');
                }} className="w-full bg-black text-white py-6 rounded-xl font-black text-[11px]">PEDIR POR WHATSAPP</button>
              </div>
            )}
          </div>
        </div>
