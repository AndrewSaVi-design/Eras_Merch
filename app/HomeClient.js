...
export default function HomeClient({ artistas = [], productos = [], banners = [], fondosMap = {} }) {
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col items-center text-black relative font-sans overflow-x-hidden">
      ...
      {/* WHATSAPP */}
      <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" className="fixed bottom-6 right-6 z-[99] hover:scale-110 transition-transform shadow-2xl rounded-full">
      ...
      <div className="w-full max-w-4xl p-6 flex flex-col items-center relative">
        ...
        <div className="flex gap-6 text-gray-400">
          ...
          <a href={links.tiktok} target="_blank" className="hover:text-black transition-colors"><TikTokIcon size={20} /></a>
        </div>
      </div>
          ...
          {banners?.length > 0 && (
            <div className="w-full max-w-5xl mb-10 group relative">
              <div className="relative aspect-[16/11] md:aspect-[21/9] w-full overflow-hidden rounded-[2.5rem] shadow-xl bg-gray-100 border border-gray-100">
                ...
                <button onClick={() => setCurrentBanner(p => (p - 1 + banners.length) % banners.length)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/30 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><ChevronLeft/></button>
                <button onClick={() => setCurrentBanner(p => (p + 1) % banners.length)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/30 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><ChevronRight/></button>
              </div>
            </div>
          )}
      ...
      {vista === 'productos' && artistaActual && (
        <div className="w-full min-h-screen relative flex flex-col items-center animate-in fade-in duration-700">
          <div className="fixed inset-0 z-[-1] bg-white" />
          {fondoUrl && (
            <div 
              className="fixed inset-0 z-[-1] bg-cover bg-center transition-all duration-1000 opacity-40"
          ...
          <div className="w-full max-w-6xl p-6 mt-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-black">
              {polosAMostrar.map(polo => (
                <div key={polo.id} className="bg-white/90 backdrop-blur-sm border border-gray-100 p-4 rounded-[2rem] flex flex-col items-center shadow-lg">
                  ...
                  <button onClick={() => agregarAlCarrito(polo)} className={`w-full py-4 rounded-xl text-[10px] font-black tracking-widest transition-all ${notificacion === polo.id ? 'bg-green-500 text-white' : 'bg-black text-white hover:bg-gray-800'}`}>
