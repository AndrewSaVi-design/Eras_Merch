...
export default function HomeClient({ artistas = [], productos = [], banners = [], fondosMap = {} }) {
  ...
  return (
    // CAMBIO CLAVE: bg-white solo si no hay vista de productos o no hay fondo
    <main className={`min-h-screen flex flex-col items-center text-black relative font-sans overflow-x-hidden ${(!urlFondo || vista === 'colecciones') ? 'bg-white' : ''}`}>
      
      {/* FONDO DINAMICO - Ahora por encima de la base pero debajo del contenido */}
      {vista === 'productos' && urlFondo && (
        <div 
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <div className="w-full max-w-4xl p-6 flex flex-col items-center relative">
          <div className="absolute right-6 top-8 cursor-pointer" onClick={() => setCarritoAbierto(true)}>
            <ShoppingBag size={26} className={vista === 'productos' && urlFondo ? 'text-white' : 'text-black'} />
          </div>
          <img src="/erasmerch.jpeg" className="w-24 mb-4" />
          <p className={`text-[12px] font-black tracking-[0.4em] mb-4 uppercase ${vista === 'productos' && urlFondo ? 'text-white' : 'text-black'}`}>ERAS MERCH</p>
        ...
        {vista === 'colecciones' && (
          <div className="w-full flex flex-col items-center px-4">
            {/* BANNER CON DESGLOSE MANUAL RECUPERADO */}
            {banners?.length > 0 && (
              <div className="w-full max-w-5xl mb-10 group relative">
                <div className="relative aspect-[16/11] md:aspect-[21/9] w-full overflow-hidden rounded-[2.5rem] shadow-xl bg-gray-50 border">
                  ...
                  {/* Flechas manuales */}
                  <button onClick={() => setCurrentBanner(p => (p - 1 + banners.length) % banners.length)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/40 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all z-20"><ChevronLeft size={24}/></button>
                  <button onClick={() => setCurrentBanner(p => (p + 1) % banners.length)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/40 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all z-20"><ChevronRight size={24}/></button>
                </div>
              </div>
            )}
        ...
        {vista === 'productos' && artistaActual && (
          <div className="w-full min-h-screen relative flex flex-col items-center">
            <div className="w-full max-w-6xl p-6 mt-10">
              <button onClick={() => setVista('colecciones')} className={`flex items-center gap-2 text-[10px] font-bold mb-10 tracking-widest uppercase ${urlFondo ? 'text-white' : 'text-gray-500'}`}><ArrowLeft size={16} /> Volver</button>
              <h2 className={`text-4xl md:text-6xl font-black text-center mb-16 uppercase italic tracking-tighter drop-shadow-lg ${urlFondo ? 'text-white' : 'text-black'}`}>{artistaActual.nombre}</h2>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {polosAMostrar.map(polo => (
                  <div key={polo.id} className="bg-white/95 border border-gray-100 p-4 rounded-[2rem] flex flex-col items-center shadow-2xl transition-transform hover:-translate-y-1">
                    ...
                    }} className="w-full py-4 rounded-xl bg-black text-white text-[10px] font-black tracking-widest hover:bg-gray-800">AÑADIR A LA BOLSA</button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
              {carrito.map((item, i) => (
                <div key={item.tempId} className="flex gap-4 items-center border-b pb-4">
                  ...
                  <button onClick={() => setCarrito(carrito.filter(c => c.tempId !== item.tempId))}><Trash2 size={18} className="text-gray-200 hover:text-red-500"/></button>
                </div>
              ))}
            </div>
            {carrito.length > 0 && (
              <div className="pt-6 border-t mt-auto">
                ...
                }} className="w-full bg-black text-white py-6 rounded-xl font-black text-[11px] uppercase">Pedir por WhatsApp</button>
              </div>
            )}
          </div>
        </div>
