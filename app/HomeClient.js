        ...
        <div className="w-full flex flex-col items-center px-4 animate-in fade-in">
          ...
          <div className="w-full max-w-2xl flex flex-col md:flex-row gap-4 mb-12">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="BUSCAR ARTISTA..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full bg-gray-50 rounded-2xl py-4 px-12 text-xs font-bold text-black placeholder:text-gray-300"/>
            </div>
      ...
      {vista === 'productos' && artistaActual && (
        <div className="w-full min-h-screen relative flex flex-col items-center animate-in fade-in">
          ...
          {artistaActual.fotoBackground && (
            <div 
              className="fixed inset-0 z-[-1] bg-cover bg-center transition-all duration-700 opacity-20"
              style={{ backgroundImage: `url(${artistaActual.fotoBackground})`, filter: 'blur(10px)' }}
            />
          )}
          ...
          <div className="w-full max-w-6xl p-6 mt-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {polosAMostrar.map(polo => (
                <div key={polo.id} className="bg-white border border-gray-100 p-4 rounded-[2rem] flex flex-col items-center shadow-lg hover:shadow-xl transition-shadow duration-300">
                  ...
                  {/* PRECIO FORZADO A NEGRO Y VISIBLE */}
                  <p className="text-black text-[15px] font-black mb-4">S/ {polo.precio}</p>
                  
                  <div className="flex gap-2 mb-4">
                    {polo.tallas.map(t => (
                      <button key={t} onClick={() => setTallaSeleccionada({...tallaSeleccionada, [polo.id]: t})} className={`w-8 h-8 rounded-full text-[9px] font-black border transition-all ${tallaSeleccionada[polo.id] === t ? 'bg-black text-white border-black' : 'text-gray-400 border-gray-200 hover:border-gray-300'}`}>{t}</button>
                    ))}
                  </div>
                  <button onClick={() => {
                  ...
                  }} className={`w-full py-4 rounded-xl text-[10px] font-black tracking-widest transition-all ${notificacion === polo.id ? 'bg-green-500 text-white' : 'bg-black text-white hover:bg-gray-800'}`}>
